...
class Beam2D:
    def __init__(self, material_properties, cross_section_parameters, points, members, boundary_conditions):
    ...
    def make_fea_model(self):
        ...
        for model_name, boundary_condition in self.boundary_conditions.items():
            models[model_name] = self.append_boundary_conditions(boundary_condition)
...
"""
class Solutions:
    def __init__(stiffness, mass, nodes, member_nodes, dofs, modes, lower_cutoff=1.0e-2):
...
class Solutions(Beam2D):
    def __init__(lower_cutoff=1.0e-2, **kwargs):
    ...
    def append_modes_to_plot(self):
        ...
        for c, (member_name, member_node_names) in enumerate(self.member_nodes.items()):
            ...
            for im, m in enumerate((1, 2)):
                ...
                lab = f"Mode {self.modes[c]}" if im==0 else ""
...
if __name__ == "__main__":
    ...
    # Beam FEA
    #beam_fea = Beam2D(**input_data["Beam FEA"]["B"])
    model_B = Solutions(**input_data["Beam FEA"]["B"])
